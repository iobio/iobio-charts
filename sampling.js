...
function sample(inRegions) {
	...
	let sampledRegions = staticSampleRegions(
		idealRegions,
		NUM_SAMPLES,
		TARGET_BIN_SIZE
	);
...
function expandRegion(region) {
	...
	const numPossibleIndices = Math.floor(
		(length - TARGET_BIN_SIZE) / TARGET_BIN_SIZE
	);
	...
	if (numPossibleIndices < 1000) {
		...
		for (let i = 0; i < NUM_SAMPLES; i++) {
			const randomStart = Math.floor(
				Math.random() * numPossibleIndices * TARGET_BIN_SIZE
			);
			samp.push({
				rname: region.rname,
				start: randomStart,
